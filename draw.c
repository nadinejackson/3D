...
void add_sphere( struct matrix * edges, 
  ...
  for (p = 0; p < points->cols; p++)
    {
      add_edge(edges, points->m[0][p],
	       points->m[1][p],
	       points->m[2][p],
	       points->m[0][p] + 1,
	       points->m[1][p],
...
struct matrix * generate_sphere(double cx, double cy, double cz,
  ...
  ident(sphere);
  struct matrix * rot = make_rotY(2 * M_PI / step);
  ...
  for (j=1; j<=step; j++)
      ...
      for ( i = 1; i <= step; i++)
	{
	  ...
	  add_point(sphere, x0, y0, 0);
	  x0 = x1;
	  y0 = y1;
	}
      
      matrix_mult(rot, sphere);
...
struct matrix * generate_torus( double cx, double cy, double cz,
  ...
  ident(torus);
  struct matrix * rot = make_rotY(2 * M_PI / step);
  struct matrix * tr = make_translate(r2, 0, 0);
  double x0, y0, x1, y1, t;
  ...
  for (j=1; j<=step; j++)
      ...
      for ( i = 1; i <= step; i++)
	{
	  ...
	  add_point(torus, x0, y0, 0);
	  x0 = x1;
	  y0 = y1;
	}

      matrix_mult(tr, torus);
      matrix_mult(rot, torus);
